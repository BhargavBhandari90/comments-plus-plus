  ...
  const BWPCPP_Script = {
    ...
    bwp_cpp_fetch_suggestion: function () {
      ...
      textarea.on("input", function () {
        let showGhostSuggestion = $("#comment-mirror").find(".ghost");
        clearTimeout(typingTimer);
        mirror.text($(this).val());
        ghost.text("").hide();
        console.log("showGhostSuggestion", showGhostSuggestion);
...
function fetchSuggestion() {
  ...
  if (fragment === "" || typeof fragment === "undefined") return;
...
function showGhostSuggestion(currentText, fullSuggestion, fullText) {
  if (!fullSuggestion.includes(currentText)) return;
